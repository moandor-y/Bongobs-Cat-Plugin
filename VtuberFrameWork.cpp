...
void VtuberFrameWork::ReanderVtuber(int targatid, char *data, int bufferWidth,
  ...
  if (isLoad) {
    ...
    if (isInit)
      VtuberDelegate::GetInstance()->Reader(targatid, data, bufferWidth,
                                            bufferheight);
  }
}
...
void VtuberFrameWork::UpData(int id, double _x, double _y, int width,
                             int height, double sc, double _delayTime,
                             bool _randomMotion, bool _break, bool _eyeBlink,
                             const char *modelPath, bool _tarck,
                             const char *mode, bool _live2d,
                             bool relative_mouse, bool _isMouseHorizontalFlip,
                             bool _isMouseVerticalFlip, bool _isUsemask) {
  VtuberDelegate::GetInstance()->UpdataViewWindow(_x, _y, width, height, sc,
                                                  id);
  VtuberDelegate::GetInstance()->updataModelSetting(
      _randomMotion, _delayTime, _break, _eyeBlink, _tarck,
      _isMouseHorizontalFlip, _isMouseVerticalFlip, id);
