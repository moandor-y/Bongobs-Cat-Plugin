...
{    
    ...
    if (settings.screen_left_override_ >= 0 && settings.screen_right_override_ >= 0 && settings.screen_right_override_ > settings.screen_left_override_) {
            width = settings.screen_right_override_ - settings.screen_left_override_;
    }
    if (settings.screen_top_override_ >= 0 && settings.screen_bottom_override_ >= 0 && settings.screen_bottom_override_ > settings.screen_top_override_) {
    ...
    if (_viewMatrix != NULL)
    {
        ...
        if (model) {
                ...
                model->UpdateTime();
		model->UpdataSetting(settings._randomMotion,
				     settings._delayTime, settings._isBreath,
				     settings._isEyeBlink, settings._isTrack,
				     settings._isMouseHorizontalFlip,
...
Csm::csmInt32 Live2DManager::ChangeScene(int id, const Csm::csmChar *_modelPath)
{
     if (strcmp(_modelPath, "") == 0)
        return -1;

        int model_id = model_data_.size() - 1;
    if (strcmp(model_data_[model_id]._modelPath.GetRawString(),
                _modelPath) == 0)
        return model_id;
