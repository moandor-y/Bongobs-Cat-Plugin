...
void View::Initialize(int id) {
  ...
  _viewData[id]._viewMatrix->SetScreenRect(
      left, right, bottom,
      top);  // デバイスに対応する画面の範囲。 Xの左端, Xの右端, Yの下端,
             // Yの上端
  ...
  _viewData[id]._viewMatrix->SetMaxScreenRect(
      ViewLogicalMaxLeft, ViewLogicalMaxRight, ViewLogicalMaxBottom,
      ViewLogicalMaxTop);
}
...
void View::UpdataViewData(int id) {
  ...
  double _x = -static_cast<float>(RenderTargetWidth - width) /
              static_cast<float>(RenderTargetWidth);
  double _y = -static_cast<float>(RenderTargetHeight - height) /
  ...
  if (_viewData[id]._mode[_mod[id]]._leftHandsCount > 0) {
    ...
    if (!isUseLive2d) {
      Live2DManager->OnUpdate(id,
                              _viewData[id]._mode[_mod[id]]._leftHandModelId);
    }
    isUp = false;
  } else if (isUseLive2d)
    isUp = false;
...
bool View::RenderRightHands(int id) {
  ...
  if (_viewData[id]._mode[_mod[id]]._rightHandsCount > 0) {
    ...
    for (auto iter = pressed_keys.rbegin(); iter != pressed_keys.rend();
         ++iter) {
      int key = TranslateKey(*iter, id) -
                _viewData[id]._mode[_mod[id]]._leftHandsCount;
    ...
    if (!isUseLive2d) {
      Live2DManager->OnUpdate(id,
                              _viewData[id]._mode[_mod[id]]._rightHandModelId);
    }
    isUp = false;
  } else if (isUseLive2d)
    isUp = false;
...
void View::InitializeSpirite(int id) {
  ...
  LAppTextureManager *textureManager =
      VtuberDelegate::GetInstance()->GetTextureManager();
  ...
  for (int _facecount = 0; _facecount < _infoReader->_faceInfo->Facecount;
       _facecount++) {
    string imageName = _infoReader->_faceInfo->FaceImages[_facecount];
    LAppTextureManager::TextureInfo *FaceTexture =
        textureManager->CreateTextureFromPngFile(maskPath + imageName);
    float x = width * 0.5f;
    float y = height * 0.5f;
    float fWidth = static_cast<float>(width * 0.5);
    float fHeight = static_cast<float>(height * 0.5);
    _viewData[id]._face[_facecount] =
        new Sprite(x, y, fWidth, fHeight, FaceTexture->id, _programId);
  }

  _viewData[id]._modecount = _infoReader->ModeCount;
  for (int _modelCount = 0; _modelCount < _infoReader->ModeCount;
       _modelCount++) {
    ...
    /***************************** backgroud
     * ***************************************/
    if (strcmp(_modeinfo[_modelCount].BackgroundImageName, "") != 0) {
      LAppTextureManager::TextureInfo *backgroundTexture =
          textureManager->CreateTextureFromPngFile(
              targetPath + _modeinfo[_modelCount].BackgroundImageName);
      float x = width * 0.5;
      float y = height * 0.5;
      float fWidth = static_cast<float>(width * 0.5);
      float fHeight = static_cast<float>(height * 0.5);
    ...
    if (strcmp(_modeinfo[_modelCount].CatBackgroundImageName, "") != 0) {
      LAppTextureManager::TextureInfo *catbackgroundTexture =
          textureManager->CreateTextureFromPngFile(
              targetPath + _modeinfo[_modelCount].CatBackgroundImageName);
      float x = width * 0.5;
      float y = height * 0.5;
      float fWidth = static_cast<float>(width * 0.5);
      float fHeight = static_cast<float>(height * 0.5);
      _viewData[id]._mode[_modelCount]._catback = new Sprite(
          x, y, fWidth, fHeight, catbackgroundTexture->id, _programId);
    } else
      _viewData[id]._mode[_modelCount]._catback = NULL;

    /***************************** keys ***************************************/
    string _keysImagePath =
        targetPath + _modeinfo[_modelCount].KeysImagePath + "/";
    _viewData[id]._mode[_modelCount]._keysCount =
        _modeinfo[_modelCount].KeysCount;

    // load keyboard
    for (int i = 0; i < _viewData[id]._mode[_modelCount]._keysCount; i++) {
      if (!_modeinfo[_modelCount].KeysImageName[i]) break;

      string imageName = _modeinfo[_modelCount].KeysImageName[i];
      LAppTextureManager::TextureInfo *KeysTexture =
          textureManager->CreateTextureFromPngFile(_keysImagePath + imageName);
      float x = width * 0.5f;
      float y = height * 0.5f;
      float fWidth = static_cast<float>(width * 0.5);
      float fHeight = static_cast<float>(height * 0.5);
    ...
    if (strcmp(_modeinfo[_modelCount].LeftHandImagePath, "") != 0) {
      string _leftHandsImagePath =
          targetPath + _modeinfo[_modelCount].LeftHandImagePath + "/";
      if (strcmp(_modeinfo[_modelCount].LeftHandUpImageName, "") != 0) {
        ...
        float fWidth = static_cast<float>(width * 0.5);
        float fHeight = static_cast<float>(height * 0.5);
        _viewData[id]._mode[_modelCount]._leftHandUp = new Sprite(
            x, y, fWidth, fHeight, LeftHandUpTexture->id, _programId);
      } else
        _viewData[id]._mode[_modelCount]._leftHandUp = NULL;

      // load left hands
      _viewData[id]._mode[_modelCount]._leftHandsCount =
          _modeinfo[_modelCount].ModelLeftHandCount;
      for (int i = 0; i < _viewData[id]._mode[_modelCount]._leftHandsCount;
           i++) {
        string imageName = _modeinfo[_modelCount].LeftHandImageName[i];
        LAppTextureManager::TextureInfo *HandsTexture =
            textureManager->CreateTextureFromPngFile(_leftHandsImagePath +
                                                     imageName);
        float x = width * 0.5f;
        float y = height * 0.5f;
        float fWidth = static_cast<float>(width * 0.5);
    ...
    if (strcmp(_modeinfo[_modelCount].RightHandImagePath, "") != 0) {
      string _rightHandsImagePath =
          targetPath + _modeinfo[_modelCount].RightHandImagePath + "/";
      if (strcmp(_modeinfo[_modelCount].RightHandUpImageName, "") != 0) {
        ...
        float fWidth = static_cast<float>(width * 0.5);
        float fHeight = static_cast<float>(height * 0.5);
        _viewData[id]._mode[_modelCount]._rightHandUp = new Sprite(
            x, y, fWidth, fHeight, RightHandUpTexture->id, _programId);
      } else
        _viewData[id]._mode[_modelCount]._rightHandUp = NULL;

      // load right hands
      _viewData[id]._mode[_modelCount]._rightHandsCount =
          _modeinfo[_modelCount].ModelRightHandCount;
      for (int i = 0; i < _viewData[id]._mode[_modelCount]._rightHandsCount;
           i++) {
        string imageName = _modeinfo[_modelCount].RightHandImageName[i];
        LAppTextureManager::TextureInfo *HandsTexture =
            textureManager->CreateTextureFromPngFile(_rightHandsImagePath +
                                                     imageName);
        float x = width * 0.5f;
        float y = height * 0.5f;
        float fWidth = static_cast<float>(width * 0.5);
...
void View::InitializeModel(int id) {
  ...
  for (int _modelCount = 0; _modelCount < _infoReader->ModeCount;
    ...
    _viewData[id]._mode[_modelCount]._haseModel =
        _modeinfo[_modelCount].HasModel;
    ...
    _viewData[id]._mode[_modelCount]._modelId = Live2DManager->ChangeScene(
        id, (_catModelPath + "/" + _catModelName).c_str());

    _viewData[id]._mode[_modelCount]._hasLeftHandModel =
        _modeinfo[_modelCount].ModelHasLeftHandModel;
    string _LeftHandModelPath =
        targetPath + _modeinfo[_modelCount].ModelLeftHandModelPath;
    string _LeftHandModelName = Pal::GetModelName(_LeftHandModelPath.c_str());
    _viewData[id]._mode[_modelCount]._leftHandModelId =
        Live2DManager->ChangeScene(
            id, (_LeftHandModelPath + "/" + _LeftHandModelName).c_str());

    _viewData[id]._mode[_modelCount]._hasRightHandModel =
        _modeinfo[_modelCount].ModelHasRightHandModel;
    string _RightHandModelPath =
        targetPath + _modeinfo[_modelCount].ModelRightHandModelPath;
    string _RightHandModelName = Pal::GetModelName(_RightHandModelPath.c_str());
    _viewData[id]._mode[_modelCount]._rightHandModelId =
        Live2DManager->ChangeScene(
            id, (_RightHandModelPath + "/" + _RightHandModelName).c_str());
  }
}
...
float View::TransformViewX(float deviceX, int id) const {
  float screenX = _viewData[id]._deviceToScreen->TransformX(
...
float View::TransformViewY(float deviceY, int id) const {
  float screenY = _viewData[id]._deviceToScreen->TransformY(
