...
void VtuberDelegate::Reader(int id, char *buffer, int bufferWidth,
                            int bufferheight) {
...
void VtuberDelegate::ChangeMode(const char *_mode, bool _live2d,
                                bool _isUseMask, int id) {
  InfoReader *_infoReader = _view->GetInfoReader();
...
VtuberDelegate::VtuberDelegate()
    : _cubismOption(), _window(NULL), ModelFileCount(0) {
  _view = new View();
...
double VtuberDelegate::getScale(int id) { return _renderInfo[id].Scale; }
