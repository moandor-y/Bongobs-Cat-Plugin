...
Csm::csmBool Model::SetupModel(ICubismModelSetting* setting) {
  ...
  if (strcmp(_modelSetting->GetModelFileName(), "") != 0) {
    ...
    if (!Pal::IsFileExist(path.GetRawString())) return false;
    ...
    for (csmInt32 i = 0; i < count; i++) {
      ...
      if (size != 0) {
        ACubismMotion* motion =
            LoadExpression(buffer, size, name.GetRawString());
...
csmBool Model::PreloadMotionGroup(const csmChar* group) {
  ...
  for (csmInt32 i = 0; i < count; i++) {
    // ex) idle_0
    csmString name = _modelSetting->GetMotionFileName(
        group,
        i);  // Utils::CubismString::GetFormatedString("%s_%d", group, i);
    csmString path = _modelSetting->GetMotionFileName(group, i);
    path = _modelHomeDir + path;

    if (_debugMode) {
      Pal::PrintLog("[APP]load motion: %s => [%s_%d] ", path.GetRawString(),
                    group, i);
    }
...
void Model::Update(Csm::csmUint16 _id) {
  ...
  if (_motionManager->IsFinished()) {
    ...
    if (randomMotion) StartRandomMotion(PriorityIdle);
  } else {
  ...
  if (isTrack) {
    _model->AddParameterValue(_idParamAngleX,
                              (1 - mouseX) * 30);  // -30から30の値を加える
    _model->AddParameterValue(_idParamAngleY, (1 - mouseY) * 30);
    _model->AddParameterValue(_idParamAngleZ,
                              (1 - mouseX) * (1 - mouseY) * -30);

    _model->AddParameterValue(
        _idParamEyeBallX, (1 - mouseX) * 30);  ///< パラメータID: ParamEyeBallX
    _model->AddParameterValue(
        _idParamEyeBallY, (1 - mouseY) * 30);  ///< パラメータID: ParamEyeBallXY
  }
...
void Model::UpdataSetting(Csm::csmBool _randomMotion,
                          Csm::csmFloat32 _delayTime, Csm::csmBool _isBreath,
                          Csm::csmBool _isEyeBlink, Csm::csmBool _isTrack,
